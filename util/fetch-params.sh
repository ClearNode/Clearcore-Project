...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/ClearCoinParams"
    else
...
Failed to fetch the ClearCoin zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
ClearCoin - fetch-params.sh

This script will fetch the ClearCoin zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common ClearCoin zkSNARK parameters. Note that it is
