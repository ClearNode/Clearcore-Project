...
#include "ui_zclrcontroldialog.h"
...
std::set<std::string> ZClrControlDialog::setSelectedMints;
std::set<CMintMeta> ZClrControlDialog::setMints;

bool CZClrControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZClrControlDialog::COLUMN_DENOMINATION || column == ZClrControlDialog::COLUMN_VERSION || column == ZClrControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZClrControlDialog::ZClrControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZClrControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZClrControlDialog::~ZClrControlDialog()
{
    delete ui;
}

void ZClrControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZClrControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZClrControlWidgetItem* itemDenom(new CZClrControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZClrControlWidgetItem *itemMint = new CZClrControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "The zCLR seed used to mint this zCLR is not the same as currently hold in the wallet";
            else
...
void ZClrControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZClrControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZClr_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZClrControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZClrControlDialog::GetSelectedMints()
{
...
void ZClrControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
