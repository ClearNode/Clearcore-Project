...
#include "qt/clr/dashboardwidget.h"
#include "qt/clr/forms/ui_dashboardwidget.h"
#include "qt/clr/sendconfirmdialog.h"
#include "qt/clr/txrow.h"
...
DashboardWidget::DashboardWidget(ClearCoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareClr, "square-chart-clr");
    setCssProperty(ui->labelSquarezClr, "square-chart-zclr");
    setCssProperty(ui->labelClr, "text-chart-clr");
    setCssProperty(ui->labelZclr, "text-chart-zclr");
    ui->labelAmountZclr->setVisible(false);
    ui->labelSquarezClr->setVisible(false);
    ui->labelZclr->setVisible(false);
    ...
    setCssProperty(ui->labelAmountClr, "text-stake-clr-disable");
    setCssProperty(ui->labelAmountZclr, "text-stake-zclr-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &ClearCoinGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &ClearCoinGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &ClearCoinGUI::openFAQ);
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isClr = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZCLR;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair CLR, zCLR
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            clr = (pair.first != 0) ? pair.first / 100000000 : 0;
            zclr = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalClr += pair.first;
            chartData->totalZclr += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesClr.append(clr);
        chartData->valueszClr.append(zclr);

        int max = std::max(clr, zclr);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(33,134,205));
    set1->setColor(QColor(49,162,241));
    ...
    set0->append(chartData->valuesClr);
    set1->append(chartData->valueszClr);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalClr > 0 || chartData->totalZclr > 0) {
        setCssProperty(ui->labelAmountClr, "text-stake-clr");
        setCssProperty(ui->labelAmountZclr, "text-stake-zclr");
    } else {
        setCssProperty(ui->labelAmountClr, "text-stake-clr-disable");
        setCssProperty(ui->labelAmountZclr, "text-stake-zclr-disable");
    }
    forceUpdateStyle({ui->labelAmountClr, ui->labelAmountZclr});
    ui->labelAmountClr->setText(GUIUtil::formatBalance(chartData->totalClr, nDisplayUnit));
    ui->labelAmountZclr->setText(GUIUtil::formatBalance(chartData->totalZclr, nDisplayUnit, true));
