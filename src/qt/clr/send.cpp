...
#include "qt/clr/forms/ui_send.h"
#include "qt/clr/addnewcontactdialog.h"
#include "qt/clr/qtutils.h"
#include "qt/clr/sendchangeaddressdialog.h"
#include "qt/clr/optionbutton.h"
#include "qt/clr/sendconfirmdialog.h"
#include "qt/clr/myaddressrow.h"
#include "qt/clr/guitransactionsutils.h"
...
SendWidget::SendWidget(ClearCoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-clr");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination clrAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(clrAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(clrAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
