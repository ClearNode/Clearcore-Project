...
#include "qt/clr/masternodeswidget.h"
#include "qt/clr/forms/ui_masternodeswidget.h"

#include "qt/clr/qtutils.h"
#include "qt/clr/mnrow.h"
#include "qt/clr/mninfodialog.h"
#include "qt/clr/masternodewizarddialog.h"
...
MasterNodesWidget::MasterNodesWidget(ClearCoinGUI *parent) :
    PWidget(parent),
    ...
    if (fs::exists(pathBootstrap)) {
        ...
        if (lineCopy.size() == 0) {
            lineCopy = "# Masternode config file\n"
                                    ...
                                    "# Example: mn1 127.0.0.2:32628 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0\n";
        }
...
void MasterNodesWidget::onCreateMNClicked()
{
    ...
    if (walletModel->getBalance() <= (COIN * GetMasterNodeCollateral(chainActive.Height()))) {
        inform(tr("Not enough balance to create a masternode, %1 %2 required.").arg(GetMasterNodeCollateral(chainActive.Height())).arg(CURRENCY_UNIT.c_str()));
        return;
    }
    showHideOp(true);
