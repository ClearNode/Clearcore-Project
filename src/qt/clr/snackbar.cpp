...
#include "qt/clr/forms/ui_snackbar.h"
#include "qt/clr/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(ClearCoinGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &ClearCoinGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
