...
#include "qt/clr/forms/ui_addresseswidget.h"
#include "qt/clr/addresslabelrow.h"
#include "qt/clr/addnewaddressdialog.h"
#include "qt/clr/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(ClearCoinGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CTxDestination clrAdd = DecodeDestination(address.toUtf8().constData(), isStakingAddress);
        ...
        if (walletModel->updateAddressBookLabels(clrAdd, label.toUtf8().constData(),
