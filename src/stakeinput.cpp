...
bool CClrStake::InitFromTxIn(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend())
        return error("%s: unable to initialize CClrStake from zerocoin spend", __func__);
...
bool CClrStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CClrStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CClrStake::GetTxOutFrom(CTxOut& out) const
{
...
CAmount CClrStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CClrStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CClrStake::GetUniqueness() const
{
...
bool CClrStake::ContextCheck(int nHeight, uint32_t nTime)
{
