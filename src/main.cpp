...
#if defined(NDEBUG)
#error "ClearCoin cannot be compiled without assertions."
#endif
                              ...
                              std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            // Reject legacy zCLR mints
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zCLR mint output in tx %s",
...
CAmount  GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 1 * COIN;
    } else if (nHeight <= 1000 && isPoSActive) {
        nSubsidy = 1 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue)
{
    ...
    if (!isPoSActive) {
    	return 0;
    } else if (nHeight <=12000 && isPoSActive){
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZCLRModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
...
DisconnectResult DisconnectBlock(CBlock& block, CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    //Track zCLR money supply
    if (!UpdateZClearCoinupplyDisconnect(block, pindex)) {
        error("%s: Failed to calculate new zCLR supply", __func__);
        return DISCONNECT_FAILED;
    }
...
{
    util::ThreadRename("clr-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (isPublicSpend) {
                    ...
                    PublicCoinSpend publicSpend(params);
                    if (!ZCLRModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                        return false;
                    }
    ...
    // Update zCLR money supply map
    if (!UpdateZClearCoinupplyConnect(block, pindex, fJustCheck)) {
        return state.DoS(100, error("%s: Failed to calculate new zCLR supply for block=%s height=%d", __func__,
                                    block.GetHash().GetHex(), pindex->nHeight), REJECT_INVALID);
    }

    // A one-time event where the zCLR supply was off (due to serial duplication off-chain on main net)
    if (Params().NetworkID() == CBaseChainParams::MAIN && pindex->nHeight == consensus.height_last_ZC_WrappedSerials + 1
        ...
        RecalculateClearCoinupply(consensus.vUpgrades[Consensus::UPGRADE_ZC].nActivationHeight, false);
    }
    ...
    // Update CLR money supply
    nMoneySupply += (nValueOut - nValueIn);
    // eliminate burnt coins amount
    if (pindex->nHeight == sporkManager.GetSporkValue(SPORK_19_BURNT_HEIGHT)){
        nMoneySupply = nMoneySupply - sporkManager.GetSporkValue(SPORK_20_BURNT_VALUE) * COIN;
        LogPrintf("Burnt at height %ld amount %ld\n", sporkManager.GetSporkValue(SPORK_19_BURNT_HEIGHT), sporkManager.GetSporkValue(SPORK_20_BURNT_VALUE));
    }
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // ClearCoin Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nClearCoinBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nClearCoinBadBlockBits)) {
        ...
        if (isBlockFromFork) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if(hasZCLRInputs && in.IsZerocoinSpend())
            ...
            if(hasZCLRInputs) {
                for (const CTxIn& zClrInput : zCLRInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zClrInput);
