...
bool CheckInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, bool fScriptChecks, unsigned int flags, bool cacheStore, std::vector<CScriptCheck>* pvChecks)
{
    if (!tx.IsCoinBase() && !tx.HasZerocoinSpendInputs()) {
        ...
        CAmount nValueIn = 0;
        CAmount nFees = 0;
        // Burn address can not transfer any coins
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == GetSporkValue(SPORK_17_BURNT_HEIGHT)){
        nMoneySupplyPrev = nMoneySupplyPrev - GetSporkValue(SPORK_18_BURNT_VALUE) * COIN;
        LogPrintf("Burnt at height %ld amount %ld\n", GetSporkValue(SPORK_17_BURNT_HEIGHT), GetSporkValue(SPORK_18_BURNT_VALUE));
    }
    /////////////////////////////////////////////////////////////////////
