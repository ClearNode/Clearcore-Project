...
#include "spork.h"
#include "zclr/deterministicmint.h"
...
UniValue getaddressinfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
                ...
                "\nReturn information about the given ClearCoin address.\n"
                ...
                "  \"account\" : \"account\"                 (string) DEPRECATED. This field will be removed in v5.0. To see this deprecated field, start clrd with -deprecatedrpc=accounts. The account associated with the address, \"\" is the default account\n"
...
UniValue getnewaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            "getnewaddress ( \"label\" )\n"
            "\nReturns a new ClearCoin address for receiving payments.\n"
...
UniValue getnewstakingaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new ClearCoin cold staking address for receiving delegated cold stakes.\n"
...
UniValue delegatoradd(const JSONRPCRequest& request)
{
    ...
    if (!IsValidDestination(dest) || isStakingAddress)
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid ClearCoin address");
    ...
    if (!keyID)
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unable to get KeyID from ClearCoin address");
...
UniValue delegatorremove(const JSONRPCRequest& request)
{
    ...
    if (!pwalletMain->HasAddressBook(keyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unable to get ClearCoin address from addressBook");
...
UniValue listdelegators(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "   \"address\": \"xxx\",  (string) ClearCoin address string\n"
            "   }\n"
            "  ...\n"
            "]\n"
...
UniValue getaccountaddress(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("getaccountaddress (Deprecated, will be removed in v5.0. To use this command, start clrd with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "getaccountaddress is deprecated and will be removed in v5.0. To use this command, start clrd with -deprecatedrpc=accounts.");
    }
...
UniValue getrawchangeaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new ClearCoin address, for receiving change.\n"
...
UniValue setlabel(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts") && request.strMethod == "setaccount") {
        if (request.fHelp) {
            throw std::runtime_error("setaccount (Deprecated, will be removed in v5.0. To use this command, start clrd with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "setaccount is deprecated and will be removed in v5.0. To use this command, start clrd with -deprecatedrpc=accounts.");
    }

    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "setlabel \"clraddress\" \"label\"\n"
            "\nSets the label associated with the given address.\n"

            "\nArguments:\n"
            "1. \"clraddress\"   (string, required) The clr address to be associated with a label.\n"
...
UniValue getaccount(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("getaccount (Deprecated, will be removed in v5.0. To use this command, start clrd with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "getaccount is deprecated and will be removed in v5.0. To use this command, start clrd with -deprecatedrpc=accounts.");
    }
...
UniValue getaddressesbyaccount(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("getaddressesbyaccount (Deprecated, will be removed in v5.0. To use this command, start clrd with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "getaddressesbyaccount is deprecated and will be removed in v5.0. To use this command, start clrd with -deprecatedrpc=accounts.");
    }
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "2. \"amount\"      (numeric, required) The amount in CLR to send. eg 0.1\n"
...
UniValue CreateColdStakeDelegation(const UniValue& params, CWalletTx& wtxNew, CReserveKey& reservekey)
{
    ...
    if (!IsValidDestination(stakeAddr) || !isStaking)
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid ClearCoin staking address");
    ...
    if (params.size() > 2 && !params[2].isNull() && !params[2].get_str().empty()) {
        ...
        if (!IsValidDestination(dest) || isStaking)
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid ClearCoin spending address");
...
UniValue delegatestake(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 6)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"stakingaddress\"      (string, required) The clr staking address to delegate.\n"
            "2. \"amount\"              (numeric, required) The amount in CLR to delegate for staking. eg 100\n"
            "3. \"owneraddress\"        (string, optional) The clr address corresponding to the key that will be able to spend the stake. \n"
...
UniValue rawdelegatestake(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "     {\n"
            "       \"value\" : x.xxx,            (numeric) The value in CLR\n"
            "       \"n\" : n,                    (numeric) index\n"
...
UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "      amount,                 (numeric) The amount in CLR\n"
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount   (numeric) The total amount in CLR received at this address.\n"
...
UniValue getreceivedbylabel(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts") && request.strMethod == "getreceivedbyaccount") {
        if (request.fHelp) {
            throw std::runtime_error("getreceivedbyaccount (Deprecated, will be removed in v5.0. To use this command, start clrd with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "getreceivedbyaccount is deprecated and will be removed in v5.0. To use this command, start clrd with -deprecatedrpc=accounts.");
    }
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in CLR received for this label.\n"
...
UniValue getbalance(const JSONRPCRequest& request)
{
    if (request.fHelp ||
        ...
        throw std::runtime_error(
            ...
            "                    To use this deprecated argument, start clrd with -deprecatedrpc=accounts."
            "2. minconf          (numeric, optional, default=1) DEPRECATED. This argument will be removed in v5.0.\n"
            "                    To use this deprecated argument, start clrd with -deprecatedrpc=accounts. Only include transactions confirmed at least this many times.\n"
            "3. includeWatchonly (bool, optional, default=false) DEPRECATED. This argument will be removed in v5.0.\n"
            "                    To use this deprecated argument, start clrd with -deprecatedrpc=accounts. Also include balance in watchonly addresses (see 'importaddress')\n"
            "4. includeDelegated (bool, optional, default=true) Only available when specifying an account.\n"
            "                    To use this argument, start clrd with -deprecatedrpc=accounts. Also include balance delegated to cold stakers\n"

            "\nResult:\n"
            "amount              (numeric) The total amount in CLR received for this account.\n"
...
UniValue getcoldstakingbalance(const JSONRPCRequest& request)
{
    if (request.fHelp || (request.params.size() > 1 && IsDeprecatedRPCEnabled("accounts")) || (request.params.size() != 0 && !IsDeprecatedRPCEnabled("accounts")))
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in CLR received for this account in P2CS contracts.\n"
...
UniValue movecmd(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("move (Deprecated, will be removed in v5.0. To use this command, start clrd with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "move is deprecated and will be removed in v5.0. To use this command, start clrd with -deprecatedrpc=accounts.");
    }
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "3. amount            (numeric, required) Quantity of CLR to move between accounts.\n"
            ...
            "\nExamples:\n"
            "\nMove 0.01 CLR from the default account to the account named tabby\n" +
            HelpExampleCli("move", "\"\" \"tabby\" 0.01") +
            "\nMove 0.01 CLR from timotei to akiko with a comment\n" +
...
UniValue sendfrom(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("sendfrom (Deprecated, will be removed in v5.0. To use this command, start clrd with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "sendfrom is deprecated and will be removed in v5.0. To use this command, start clrd with -deprecatedrpc=accounts.");
    }

    if (request.fHelp || request.params.size() < 3 || request.params.size() > 7)
        throw std::runtime_error(
            "sendfrom \"fromaccount\" \"toclraddress\" amount ( minconf \"comment\" \"comment-to\" includeDelegated)\n"
            "\nDEPRECATED (use sendtoaddress). Send an amount from an account to a clr address.\n"
            ...
            "3. amount                (numeric, required) The amount in CLR. (transaction fee is added on top).\n"
            "4. minconf               (numeric, optional, default=1) Only use funds with at least this many confirmations.\n"
            ...
            "\nExamples:\n"
            "\nSend 0.01 CLR from the default account to the address, must have at least 1 confirmation\n" +
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("accounts")) {
        help_text = "sendmany \"\" {\"address\":amount,...} ( minconf \"comment\" includeDelegated )\n"
            ...
            "clrd with -deprecatedrpc=accounts\n" +
            HelpRequiringPassphrase() + "\n"
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The clr address is the key, the numeric amount in CLR is the value\n"
            "      ,...\n"
            "    }\n"
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!IsValidDestination(dest) || isStaking)
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid ClearCoin address: ")+name_);
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "Each key is a ClearCoin address or hex-encoded public key.\n"
...
UniValue listreceivedbyaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (numeric) The total amount in CLR received by the address\n"
...
UniValue listreceivedbylabel(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts") && request.strMethod == "listreceivedbyaccount") {
        if (request.fHelp) {
            throw std::runtime_error("listreceivedbyaccount (Deprecated, will be removed in v5.0. To use this command, start clrd with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "listreceivedbyaccount is deprecated and will be removed in v5.0. To use this command, start clrd with -deprecatedrpc=accounts.");
    }
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("accounts")) {
        help_text = "listtransactions ( \"dummy\" count from includeWatchonly includeDelegated )\n"
            ...
            "    \"amount\": x.xxx,          (numeric) The amount in CLR. This is negative for the 'send' category, and for the\n"
            "                                         'move' category for moves outbound. It is positive for the 'receive' category,\n"
            "                                         and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in CLR. This is negative and only available for the \n"
...
UniValue listaccounts(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("listaccounts (Deprecated, will be removed in v5.0. To use this command, start clrd with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "listaccounts is deprecated and will be removed in v5.0. To use this command, start clrd with -deprecatedrpc=accounts.");
    }
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "{\n"
            "  \"transactions\": [\n"
            "    \"account\":\"accountname\",       (string) DEPRECATED. This field will be removed in v5.0. To see this deprecated field, start clrd with -deprecatedrpc=accounts. The account name associated with the transaction. Will be \"\" for the default account.\n"
            ...
            "    \"amount\": x.xxx,          (numeric) The amount in CLR. This is negative for the 'send' category, and for the 'move' category for moves \n"
            "                                          outbound. It is positive for the 'receive' category, and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in CLR. This is negative and only available for the 'send' category of transactions.\n"
...
UniValue gettransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"amount\" : x.xxx,        (numeric) The transaction amount in CLR\n"
            ...
            "  \"details\" : [\n"
            "    {\n"
            "      \"account\" : \"accountname\",  (string) DEPRECATED.This field will be removed in v5.0. To see this deprecated field, start clrd with -deprecatedrpc=accounts. The account name involved in the transaction, can be \"\" for the default account.\n"
            "      \"address\" : \"clraddress\",   (string) The clr address involved in the transaction\n"
            "      \"category\" : \"send|receive\",    (string) The category, either 'send' or 'receive'\n"
            "      \"amount\" : x.xxx                  (numeric) The amount in CLR\n"
            "      \"vout\" : n,                       (numeric) the vout value\n"
            "    }\n"
            "    ,...\n"
            "  ],\n"
...
UniValue walletpassphrase(const JSONRPCRequest& request)
{
    if (pwalletMain->IsCrypted() && (request.fHelp || request.params.size() < 2 || request.params.size() > 3))
        throw std::runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending CLRs\n"
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    if (!pwalletMain->IsCrypted() && (request.fHelp || request.params.size() != 1))
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending CLRs\n" +
...
UniValue listunspent(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
                ...
                "    \"account\" : \"account\",  (string) DEPRECATED.This field will be removed in v5.0. To see this deprecated field, start clrd with -deprecatedrpc=accounts. Backwards compatible alias for label.\n"
    ...
    if (request.params.size() > 2) {
        ...
        for (unsigned int inx = 0; inx < inputs.size(); inx++) {
            ...
            if (!IsValidDestination(dest))
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid ClearCoin address: ") + input.get_str());
...
UniValue lockunspent(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending CLRs.\n"
...
UniValue settxfee(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in CLR/kB rounded to the nearest 0.00000001\n"
...
UniValue getwalletinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,                      (numeric) the total CLR balance of the wallet (cold balance excluded)\n"
            "  \"delegated_balance\": xxxxx,              (numeric) the CLR balance held in P2CS (cold staking) contracts\n"
            "  \"cold_staking_balance\": xx,              (numeric) the CLR balance held in cold staking addresses\n"
            "  \"unconfirmed_balance\": xxx,              (numeric) the total unconfirmed balance of the wallet in CLR\n"
            "  \"immature_delegated_balance\": xxxxxx,    (numeric) the delegated immature balance of the wallet in CLR\n"
            "  \"immature_cold_staking_balance\": xxxxxx, (numeric) the cold-staking immature balance of the wallet in CLR\n"
            "  \"immature_balance\": xxxxxx,              (numeric) the total immature balance of the wallet in CLR\n"
            ...
            "  \"paytxfee\": x.xxxx                       (numeric) the transaction fee configuration, set in CLR/kB\n"
...
UniValue setstakesplitthreshold(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "coinstake transaction will have 4 outputs (of 500 CLR each)."
            + HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. value                   (numeric, required) Threshold value (in CLR).\n"
...
UniValue autocombinerewards(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || (fEnable && request.params.size() != 2) || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nWallet will automatically monitor for any coins with value below the threshold amount, and combine them if they reside with the same ClearCoin address\n"
...
UniValue multisend(const JSONRPCRequest& request)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid CLR address");
...
UniValue getzerocoinbalance(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            "getzerocoinbalance\n"
            "\nReturn the wallet's total zCLR balance.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nResult:\n"
            "amount         (numeric) Total zCLR balance.\n"
...
UniValue listmintedzerocoins(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nList all zCLR mints in the wallet.\n" +
            HelpRequiringPassphrase() + "\n"
            ...
            "    \"zCLR ID\": \"xxx\",   (string) Pubcoin in hex format.\n"
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    std::set<CMintMeta> setMints = pwalletMain->zclrTracker->ListMints(true, fMatureOnly, true);
    ...
    if (fVerbose) {
        for (auto m : setMints) {
            ...
            objMint.push_back(Pair("zCLR ID", m.hashPubcoin.GetHex()));     // PubCoin
...
UniValue listzerocoinamounts(const JSONRPCRequest& request)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    std::set<CMintMeta> setMints = pwalletMain->zclrTracker->ListMints(true, true, true);
...
UniValue listspentzerocoins(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            "listspentzerocoins\n"
            "\nList all the spent zCLR mints in the wallet.\n" +
            HelpRequiringPassphrase() + "\n"
...
UniValue mintzerocoin(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nMint the specified zCLR amount\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. amount      (numeric, required) Enter an amount of Clr to convert to zCLR\n"
    ...
    if (!Params().IsRegTestNet())
        throw JSONRPCError(RPC_WALLET_ERROR, "zCLR minting is DISABLED");
...
UniValue spendzerocoin(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 2 || request.params.size() < 1)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"value\": amount,         (numeric) Value in CLR.\n"
            "      \"address\": \"xxx\"         (string) CLR address or \"zerocoinmint\" for reminted change.\n"
            "    }\n"
            "    ,...\n"
            "  ]\n"
            "}\n"
    ...
    return DoZclrSpend(nAmount, vMintsSelected, address_str);
}
...
UniValue spendzerocoinmints(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nSpend zCLR mints to a CLR address.\n" +
            HelpRequiringPassphrase() + "\n"
...
extern UniValue DoZclrSpend(const CAmount nAmount, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str)
{
...
UniValue resetmintzerocoin(const JSONRPCRequest& request)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzCLRTracker* zclrTracker = pwalletMain->zclrTracker.get();
    std::set<CMintMeta> setMints = zclrTracker->ListMints(false, false, true);
...
UniValue resetspentzerocoin(const JSONRPCRequest& request)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzCLRTracker* zclrTracker = pwalletMain->zclrTracker.get();
    std::set<CMintMeta> setMints = zclrTracker->ListMints(false, false, false);
...
UniValue exportzerocoins(const JSONRPCRequest& request)
{
    if(request.fHelp || request.params.empty() || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"denomination\"         (integer, optional) Export a specific denomination of zCLR\n"

            "\nResult:\n"
            "[                   (array of json object)\n"
            "  {\n"
            "    \"id\": \"serial hash\",  (string) the mint's zCLR serial hash \n"
            ...
            "    \"v\": version,   (numeric) The version of the zCLR\n"
            "    \"k\": \"privkey\"  (string) The zCLR private key (V2+ zCLR only)\n"
            "  }\n"
            "  ,...\n"
            "]\n"
    ...
    CzCLRTracker* zclrTracker = pwalletMain->zclrTracker.get();
    std::set<CMintMeta> setMints = zclrTracker->ListMints(!fIncludeSpent, false, false);
...
UniValue importzerocoins(const JSONRPCRequest& request)
{
    if(request.fHelp || request.params.size() == 0)
        throw std::runtime_error(
            ...
            "  \"value\": amount    (numeric) The total zCLR value of zerocoin mints that were added\n"
            "}\n"
...
UniValue reconsiderzerocoins(const JSONRPCRequest& request)
{
    if(request.fHelp || !request.params.empty())
        throw std::runtime_error(
            "reconsiderzerocoins\n"
            "\nCheck archived zCLR list to see if any mints were added to the blockchain.\n" +
            HelpRequiringPassphrase() + "\n"
...
UniValue setzclreed(const JSONRPCRequest& request)
{
    if(request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "setzclreed \"seed\"\n"
            ...
            "\nExamples\n" +
            HelpExampleCli("setzclreed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5") +
            HelpExampleRpc("setzclreed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5"));
    ...
    CzCLRWallet* zwallet = pwalletMain->getZWallet();
...
UniValue getzclreed(const JSONRPCRequest& request)
{
    if(request.fHelp || !request.params.empty())
        throw std::runtime_error(
            "getzclreed\n"
            ...
            "\nResult\n"
            "\"seed\" : s,  (string) The deterministic zCLR seed.\n"

            "\nExamples\n" +
            HelpExampleCli("getzclreed", "") + HelpExampleRpc("getzclreed", ""));
...
UniValue generatemintlist(const JSONRPCRequest& request)
{
    if(request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "generatemintlist\n"
            "\nShow mints that are derived from the deterministic zCLR seed.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments\n"
            "1. \"count\"  : n,  (numeric) Which sequential zCLR to start with.\n"
            "2. \"range\"  : n,  (numeric) How many zCLR to generate.\n"
...
UniValue dzclrtate(const JSONRPCRequest& request) {
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
                "dzclrtate\n"
                        "\nThe current state of the mintpool of the deterministic zCLR wallet.\n" +
                HelpRequiringPassphrase() + "\n"
...
void static SearchThread(CzCLRWallet* zwallet, int nCountStart, int nCountEnd)
{
    ...
    try {
        ...
        for(int i = nCountStart; i < nCountEnd; i++) {
            ...
            CKey key;
            zwallet->SeedToZCLR(zerocoinSeed, bnValue, bnSerial, bnRandomness, key);
...
UniValue searchdzclr(const JSONRPCRequest& request)
{
    if(request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            "searchdzclr\n"
            "\nMake an extended search for deterministically generated zCLR that have not yet been recognized by the wallet.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments\n"
            "1. \"count\"       (numeric) Which sequential zCLR to start with.\n"
            "2. \"range\"       (numeric) How many zCLR to generate.\n"
...
UniValue spendrawzerocoin(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 4 || request.params.size() > 6)
        throw std::runtime_error(
            ...
            "5. \"address\"          (string, optional) ClearCoin address to spend to. If not specified, "
    ...
    return DoZclrSpend(mint.GetDenominationAsAmount(), vMintsSelected, address_str);
}
