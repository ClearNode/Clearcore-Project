    ...
    if(IsCrypted()) {
        if (!IsLocked()) { //if we have password
            ...
            if (EncryptSecret(vMasterKey, kmSeed, hashSeed, vchSeedSecret)) {
                ...
                if (db.WriteZClearCoineed(hashSeed, vchSeedSecret)) {
                    return true;
                }
            }
            strErr = "encrypt seed";
        ...
        if (db.WriteZClearCoineed(hashSeed, ToByteVector(seed))) {
            return true;
        }
        strErr = "save zclreed to wallet";
    }
    //the use case for this is no password set seed, mint dzCLR,
...
bool CWallet::GetDeterministicSeed(const uint256& hashSeed, uint256& seedOut)
{
    ...
    if (IsCrypted()) {
        if(!IsLocked()) { //if we have password
            ...
            //read encrypted seed
            if (db.ReadZClearCoineed(hashSeed, vchCryptedSeed)) {
        ...
        // wallet not crypted
        if (db.ReadZClearCoineed(hashSeed, vchSeed)) {
...
void CWallet::doZClrRescan(const CBlockIndex* pindex, const CBlock& block,
        std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckZCLR)
{
    //If this is a zapwallettx, need to read zpiv
    if (fCheckZCLR && consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::UPGRADE_ZC)) {
...
bool CWallet::CreateZCLROutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwallet->GenerateDeterministicZCLR(denomination, coin, dMint);
                                    ...
                                    CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    for (auto &it : mapMintsSelected) {
        ...
        if (!GetTransaction(mint.GetTxHash(), txMint, hashBlock)) {
            receipt.SetStatus(strprintf(_("Unable to find transaction containing mint %s"), mint.GetTxHash().GetHex()), ZCLR_TXMINT_GENERAL);
            return false;
        } else if (mapBlockIndex.count(hashBlock) < 1) {
            // check that this mint made it into the blockchain
            receipt.SetStatus(_("Mint did not make it into blockchain"), ZCLR_TXMINT_GENERAL);
            return false;
        }
        ...
        if (outputIndex == -1) {
            receipt.SetStatus(_("Pubcoin not found in mint tx"), ZCLR_TXMINT_GENERAL);
            return false;
        }

        mint.SetOutputIndex(outputIndex);
        CTxIn in;
        if(!ZCLRModule::createInput(in, mint, hashTxOut, spendVersion)) {
            receipt.SetStatus(_("Cannot create public spend input"), ZCLR_TXMINT_GENERAL);
            return false;
        }
        vin.emplace_back(in);
        ...
        CTxDestination * changeAddress)
{
    ...
    const int nMaxSpends = Params().GetConsensus().ZC_MaxPublicSpendsPerTx; // Maximum possible spends for one zCLR public spend transaction
    std::vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the zCLR used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
        setMints = zclrTracker->ListMints(true, false, true, true); // need to find mints to spend
        if(setMints.empty()) {
...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use zCLR in the future, for now it's useless,
        ...
        CAmount nBalance = 0;
        std::vector<CMintMeta> vMints = zclrTracker->GetMints(true);
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        std::set<CMintMeta> setMints = zclrTracker->ListMints(true, true, true);
...
void CWallet::setZWallet(CzCLRWallet* zwallet)
{
    this->zwallet = zwallet;
    zclrTracker = std::unique_ptr<CzCLRTracker>(new CzCLRTracker(this));
}

CzCLRWallet* CWallet::getZWallet()
{
    return zwallet;
}
