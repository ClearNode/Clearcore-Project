...
#include "zclr/zclrwallet.h"


CzCLRTracker::CzCLRTracker(CWallet* parent)
{
    this->wallet = parent;
...
{
    bool iszCLRWalletInitialized = (nullptr != zCLRWallet);
    CMintMeta meta;
    ...
    if (!iszCLRWalletInitialized)
        zCLRWallet = new CzCLRWallet(wallet);
...
std::set<CMintMeta> CzCLRTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dzclr from DB\n", __func__, listDeterministicDB.size());
    }
