...
CzCLRWallet::CzCLRWallet(CWallet* parent)
{
    this->wallet = parent;
...
void CzCLRWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->zclrTracker->HasPubcoinHash(pMint.first)) {
...
bool CzCLRWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    wallet->zclrTracker->Add(dMint, true);
...
void CzCLRWallet::GenerateDeterministicZCLR(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCLRWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
