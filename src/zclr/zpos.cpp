...
bool CLegacyZClrStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZClrStake from non zc-spend", __func__);
    ...
    *this = CLegacyZClrStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zclr stake origin", __func__);

    // All good
    return true;
}

CLegacyZClrStake::CLegacyZClrStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZClrStake::GetIndexFrom()
{
...
CAmount CLegacyZClrStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZClrStake::GetUniqueness() const
{
...
bool CLegacyZClrStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zCLR stake block: height %d outside range", __func__, nHeight);
