...
class ReorgStakeTest(ClearCoinTestFramework):
    ...
    def check_money_supply(self, expected_clr, expected_zclr):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected CLR and zCLR supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_clr))
            for denom in node['zCLRsupply']:
                assert_equal(node['zCLRsupply'][denom], DecimalAmt(expected_zclr[denom]))
    ...
    def run_test(self):
        ...
        # Check CLR and zCLR supply at the beginning
        # ------------------------------------------
        # zCLR supply: 2 coins for each denomination
        expected_zclr_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # CLR supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zclr_supply)
        ...
        # Verify that CLR and zCLR supplies were properly updated after the spends and reorgs
        self.log.info("Check CLR and zCLR supply...")
        ...
        expected_zclr_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zclr_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zclr_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zclr_supply)
