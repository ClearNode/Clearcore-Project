...
class ZerocoinSpendTest(ClearCoinTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zclr_bal, clr_bal):
            zclr_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zclr_bal)
            clr_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], clr_bal)
            return zclr_bal, clr_bal
        ...
        zclr_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zclr_balance, 6666)
        ...
        zclr_balance, balance = check_balances(denom_2, zclr_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zclr_balance, balance = check_balances(denom_3, zclr_balance, balance)
        ...
        _, _ = check_balances(denom_1, zclr_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
