...
*Setup instructions for a gitian build of ClearCoin Core using a VM or physical system.*
...
to the ClearCoin GitHub release page.
...
- [Building ClearCoin Core](#building-clr-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/clr-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/clr-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/clr-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/clr-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/clr-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/clr-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/ClearNode/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@clearnodes.com) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/clr-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/clr-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/clr-osx-*-build.assert
```
