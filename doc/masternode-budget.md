...
ClearCoin now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<clr_address\> \<monthly_payment_clr\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ ClearCoin, _12_ times over the course of a year totaling _24000_ ClearCoin.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<clr_address\> \<monthly_payment_clr\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` ClearCoin.
 ...
 * vote-many          - Vote on a ClearCoin initiative
 * vote-alias         - Vote on a ClearCoin initiative
 * vote               - Vote on a ClearCoin initiative/budget
