...
The build script we'll be using is contained in the ClearCoin github repository ([contrib/gitian-build.py](https://github.com/ClearNode/ClearCoin/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the ClearCoin repository and will need to fetch this script with the following commands:
...
- Clone the gitian-builder, gitian.sigs, clr-detached-sigs, and clr GitHub repos.
